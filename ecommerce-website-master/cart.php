...
<head>
    ...
    <title>As Salih Production | Impression et Vente de Matériel Informatique</title>
...
if (mysqli_num_rows($result) >= 1) {
    ?>
                        <thead>
                            <tr>
                                <th>Numéro D'articles</th>
                                <th>Nom D'articles</th>
                                <th>Prix</th>
                                <th></th>
                            </tr>
                        </thead>
...
while ($row = mysqli_fetch_array($result)) {
        $sum += $row["Price"];
        $id = $row["id"] . ", ";
        echo "<tr><td>" . "#" . $row["id"] . "</td><td>" . $row["Name"] . "</td><td>FCFA " . $row["Price"] . "</td><td><a href='cart-remove.php?id={$row['id']}' class='remove_item_link'> Supprimer</a></td></tr>";
    }
    $id = rtrim($id, ", ");
    echo "<tr><td></td><td>Total</td><td>FCFA " . $sum . "</td><td><a href='success.php' class='btn btn-primary'>Confirmer votre Commande</a></td></tr>";
    ?>
                            </tbody>
                            <?php
} else {
